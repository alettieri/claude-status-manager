...
After writing the plan file, import it into the status-manager database (if the status-manager system is in use):

```bash
sm plan import <worktree-name> --file <path-to-plan>
```

The worktree name is the short identifier for the current git worktree. This makes the plan available to the `/execute` skill via `sm plan show` and visible on the dashboard. If the status-manager server is not running or the worktree is not registered, skip this step and note it to the user.
...
1. **Task subject**
   ...
   #### Acceptance criteria
   - [ ] Task-level criterion 1
   - [ ] Task-level criterion 2
